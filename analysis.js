...
async function loadDailyChart() {
    ...
    const dailyData = (USE_MOCK ? MOCK_INPUT_DATA : await fetchData('hourly'))
        ...
        .map(item => ({ hour: new Date(item._id).getUTCHours() + ':00', count: item.count }));

    const allHours = Array.from({ length: 24 }, (_, i) => `${i}:00`);
    const hourlyCounts = {};
    dailyData.forEach(item => { hourlyCounts[item.hour] = (hourlyCounts[item.hour] || 0) + item.count; });
...
async function callGemini() {
    ...
    const payload = {
        contents: [
            { role: "user", parts: [{ text: "You are an agent that can analyze the following data: " + JSON.stringify(MOCK_INPUT_DATA, null, 2) + ". Based on these data, please provide a short, friendly analysis and advice on how to relieve stress, keeping your answer under 3 paragraphs." }] }
        ],
    ...
    try {
        ...
        document.getElementById("geminiOutput").innerHTML = fullText ? fullText.replace(/\n/g, "<br>") : "No useful content returned.";
    } catch (error) {
